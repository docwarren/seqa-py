...
class TestVcfLocal:
    def test_returns_correct_line_count(self):
        ...
        lines = file_search(LOCAL_VCF, "chr12:1-100000000", include_header=False)
    ...
    def test_include_header_true(self):
        ...
        lines = file_search(LOCAL_VCF, "chr12:1-100000000", include_header=True)
    ...
    def test_header_only(self):
        lines = file_search(LOCAL_VCF, "chr12:1-100000000", header_only=True)
        assert len(lines) > 0
    ...
    def test_empty_region_returns_no_lines(self):
        ...
        lines = file_search(LOCAL_VCF, "chr12:1-100", include_header=False)
        assert lines == []

    def test_coordinate_formats(self):
        """All supported coordinate formats should return the same result."""
        with_range = file_search(LOCAL_VCF, "chr12:1-100000000", include_header=False)
        with_commas = file_search(LOCAL_VCF, "chr12:1-100,000,000", include_header=False)
        assert with_range == with_commas

    def test_accepts_absolute_path(self):
        abs_path = os.path.abspath(LOCAL_VCF)
        lines = file_search(abs_path, "chr12:1-100000000", include_header=False)
        assert len(lines) == 250
    ...
    def test_invalid_extension_raises(self):
        ...
        with pytest.raises(ValueError):
            file_search(LOCAL_VCF.replace(".vcf.gz", ".unknown"), "chr12:1-100000000")

    def test_nonexistent_file_raises(self):
        with pytest.raises(ValueError):
            file_search("/nonexistent/path/file.vcf.gz", "chr12:1-100000000")

    def test_invalid_chromosome_raises(self):
        with pytest.raises(ValueError, match="Invalid Coordinate string format"):
            file_search(LOCAL_VCF, "chrZZZ:1-1000")
